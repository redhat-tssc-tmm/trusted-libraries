...
This script provides complete end-to-end verification of Python packages from
Red Hat Trusted Libraries. It combines all verification steps into a single tool.

What does this script verify?
-----------------------------
1. Wheel Hash: Downloads the wheel and verifies its SHA256 matches the index
2. Attestation Hash: Verifies the attestation's subject digest matches the wheel
3. Attestation Signature: Uses cosign to verify the DSSE signature (RSA-4096)
4. Installed Files: Verifies installed files match the wheel's RECORD hashes

What is an attestation?
-----------------------
An attestation is a cryptographically signed statement about how a software
artifact was built. It follows the in-toto specification and contains:
  - Subject: The file being attested (wheel name + SHA256 hash)
  - Predicate: Build metadata (who built it, when, how)
  - Signature: RSA-4096 signature proving authenticity

What is DSSE?
-------------
DSSE (Dead Simple Signing Envelope) is the format used to sign attestations.
The signature is over a PAE (Pre-Authentication Encoding) that includes the
payload type and length, preventing type confusion and length extension attacks.

What is RECORD?
---------------
Every wheel contains a RECORD file listing all files with their SHA256 hashes.
By verifying installed files against the RECORD from a verified wheel (not the
installed RECORD), we can detect any post-installation tampering.
...
Usage:
    ...
    python verify_package_provenance.py --verbose pyyaml  # Show all details
"""
...
#
# The signature is over a PAE (Pre-Authentication Encoding), NOT the raw payload.
# PAE format: "DSSEv1 {type_len} {type} {payload_len} {payload}"
#
# This prevents several attacks:
#   1. Type confusion: Attacker can't reuse signatures from different contexts
#   2. Length extension: Attacker can't append data to signed messages
#   3. Ambiguity: Only one way to interpret the signed data
#
...
# =============================================================================
#
# Why verify against the wheel's RECORD, not the installed RECORD?
# ----------------------------------------------------------------
# When pip installs a wheel, it also extracts the RECORD file. An attacker
# who modifies installed files could also update the RECORD to match.
# By using the RECORD from the verified wheel (whose hash matches the signed
# attestation), we get the authentic file hashes from the build system.
#
# RECORD file format (CSV):
# -------------------------
#   path,hash,size
#   package/__init__.py,sha256=abc123...,1234
#   package/module.py,sha256=def456...,5678
#   package-1.0.dist-info/RECORD,,          <- RECORD itself has no hash
#
# The hash uses base64url encoding (RFC 4648):
#   - Uses '-' instead of '+'
#   - Uses '_' instead of '/'
...
def get_default_public_key_path() -> Path:
    """Get the default public key path (redhat-release3.pub in repository root)."""
    script_dir = Path(__file__).parent.resolve()
    # Go up two levels: blog/scripts -> blog -> pulp-index (repo root)
